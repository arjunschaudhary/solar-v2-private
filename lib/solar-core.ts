...
export function demoWorkspace(now=new Date().toISOString()):Workspace{
 ...
 rows.forEach((r,i)=>{s=applyAction(s,{type:'create_lead',request_id:'demo-'+i,payload:{client_name:r[0],company_name:r[1],email:'demo'+i+'@example.com',location:r[3],project_type:r[2],estimated_capacity:r[4],budget_range:i<4?'₹10–25 lakhs':'Not sure',timeline:i<3?'Within 1 month':'1-3 months',site_visit_required:i%2?'No':'Yes',electricity_bill_available:'Yes',message:'Please help us plan the next steps for this solar project.'}},'Sample workspace',now).state;s.followups[i].followup_due_date=addDays(day(now),i<2?-1:i<5?0:2);s.leads[i].next_followup_date=s.followups[i].followup_due_date;});
 s=applyAction(s,{type:'schedule_visit',request_id:'demo-visit',payload:{lead_id:s.leads[0].lead_id,assigned_engineer:'Amit Verma',starts_at:new Date(Date.parse(now)+86400000).toISOString(),required_documents:'Electricity bill and roof layout'}},'Sample workspace',now).state;
 s=applyAction(s,{type:'create_proposal',request_id:'demo-proposal',payload:{lead_id:s.leads[2].lead_id,amount:4200000,notes:'250 kW rooftop installation. Scope includes panels, inverters, mounting structures and installation. Sample estimate only.'}},'Sample workspace',now).state;
 return s;
}
